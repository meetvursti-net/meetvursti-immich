<script lang="ts">
  ...
  // Derived accessors for filtered activities (separates album-level from asset-level)
  let displayActivities = $derived(activityManager.displayActivities);
  let displayCommentCount = $derived(activityManager.displayCommentCount);
  ...
  // Handle comment reaction - sets or toggles the user's reaction
  const handleCommentReaction = async (commentId: string, emoji: string) => {
    try {
      await activityManager.setCommentReaction(commentId, emoji);
  ...
  // Toggle off comment reaction when clicking on it
  const handleCommentReactionToggle = async (commentId: string, emoji: string) => {
    try {
      const userReaction = activityManager.getUserCommentReaction(commentId);
      if (userReaction && userReaction.reaction === emoji) {
        // User clicked their own reaction - remove it
        await activityManager.setCommentReaction(commentId, null);
      } else {
        // User clicked a different reaction - change to it
        await activityManager.setCommentReaction(commentId, emoji);
      }
      showCommentReactionDropdown = null;
    } catch (error) {
      handleError(error, $t('errors.unable_to_add_reaction'));
    }
  };
...
<div class="flex flex-col h-full border-l border-subtle bg-light">
  <!-- Header -->
  <div
    class="flex-shrink-0 flex w-full dark:text-immich-dark-fg p-2 bg-light border-b border-gray-200 dark:border-gray-700"
  >
  ...
  <!-- Middle section: fills remaining space between header and input -->
  <div class="flex-1 min-h-0 flex flex-col">
    <!-- Scrollable content area -->
    <div class="flex-1 overflow-y-auto immich-scrollbar w-full px-2">
        ...
        <div class="flex items-center justify-between py-3 px-2 border-b border-gray-200 dark:border-gray-700">
          ...
          <div class="flex items-center gap-1 flex-wrap">
            {#if activityManager.reactionSummary.length > 0}
              {#each activityManager.reactionSummary as { reaction, count, activities }}
                    ...
                    <div class="absolute top-full left-0 mt-2 z-50">
                      ...
                      >
                        ...
                        <div class="flex flex-col gap-1 max-h-48 overflow-y-auto">
                          {#each activities as reactionActivity}
                            <button
                              type="button"
                              class="flex items-center gap-2 p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors w-full text-left"
                              onclick={() => {
                                showReactionDropdown = null;
                                goto(Route.viewPartner({ id: reactionActivity.user.id }));
                              }}
                            >
                              <UserAvatar user={reactionActivity.user} size="sm" />
                              <span class="text-sm text-immich-fg dark:text-immich-dark-fg truncate"
                                >{reactionActivity.user.name}</span
                              >
                            </button>
                          {/each}
                        </div>
      ...
      {#if displayCommentCount > 0}
        <div class="py-2 px-2">
          <span class="text-sm font-medium text-gray-600 dark:text-gray-400">
            {$t('comments')} ({displayCommentCount})
          </span>
        </div>
      {/if}

      {#each displayActivities as reaction, index (reaction.id)}
        {#if reaction.type === ReactionType.Comment}
            ...
            <div class="flex gap-3 justify-start">
              ...
              <div class="flex-1 min-w-0">
                ...
                {#if activityManager.getCommentReactionDetails(reaction.id).length > 0}
                  {@const userCommentReaction = activityManager.getUserCommentReaction(reaction.id)}
                  <div class="flex gap-1 mt-2 flex-wrap">
                    {#each activityManager.getCommentReactionDetails(reaction.id) as { reaction: emoji, count, activities }}
                      {@const isUserReaction = userCommentReaction?.reaction === emoji}
                      <div
                        class="relative"
                      ...
                      >
                        <button
                          type="button"
                          class="text-xs rounded-full px-2 py-0.5 transition-colors {isUserReaction
                            ? 'bg-primary/30 dark:bg-primary/40 ring-1 ring-primary'
                            : 'bg-gray-300 dark:bg-gray-700 hover:bg-gray-400 dark:hover:bg-gray-600'}"
                          onclick={() => handleCommentReactionToggle(reaction.id, emoji)}
                          title={isUserReaction ? $t('click_to_remove_reaction') : $t('click_to_react')}
                        >
                          {emoji}
                          {count}
                        </button>
                ...
                <div class="flex items-center gap-2 mt-2">
                  <!-- React button - temporarily disabled due to overflow clipping issue
                  {#if true}
                    {@const userReaction = activityManager.getUserCommentReaction(reaction.id)}
                    <div class="relative" use:clickOutside={{ onOutclick: () => (showEmojiPickerForComment = null) }}>
                      <button
                        type="button"
                        class="text-xs flex items-center gap-1 {userReaction
                          ? 'text-primary'
                      ...
                      >
                        {#if userReaction}
                          <span class="text-sm">{userReaction.reaction}</span>
                        {:else}
          ...
          {#if (index != displayActivities.length - 1 && !shouldGroup(displayActivities[index].createdAt, displayActivities[index + 1].createdAt)) || index === displayActivities.length - 1}
            <div
        ...
        {:else if reaction.type === ReactionType.Like}
          <div class="relative">
            ...
            {#if (index != displayActivities.length - 1 && isTenMinutesApart(displayActivities[index].createdAt, displayActivities[index + 1].createdAt)) || index === displayActivities.length - 1}
              <div
                class="pt-1 px-2 text-right w-full text-sm text-gray-500 dark:text-gray-300"
    ...
    <!-- Chat input at bottom of middle section -->
    <div class="flex-shrink-0 w-full bg-light border-t border-gray-200 dark:border-gray-700">
